...
// Partial
  ...
  type PartialProps2 = Partial<PropsPartial> // 新类型PartialProps结构和PropsPartial相同,但所有属性变为可选
...
// Readonly
  ...
  type ReadonlyProps2 = Readonly<PropsReadonly> // 新类型PartialProps结构和PropsReadonly相同,但所有属性变为只读
  ...
  // Pick
    ...
    type PickProps = Pick<PropsPick, 'id' | 'children'> // 构造出来的类型,只有id|children两个属性类型

// Record<Keys, Type> 构造一个对象类型,属性键为keys,属性类型为Type
    type RecordObj = Record<'a' | 'b' | 'c', string[]>
    type RecordObj2 = Record<'a' | 'b' | 'c', string[]>
    let objRecord: RecordObj = {
      a: ['jack'],
      b: ['rose'],
      c: ['tar ']
    }
