...
// 当无法确定对象中有哪些属性，或者对象可以出现任意多个属性
  // [key: string]来约束属性名称 obj可以出现任意多个属性
  interface AnyObject {
    [key: string]: number // key只是个占位符,可以任意变量名称 js对象({})的键是string类型 
  }
  let obj: AnyObject = {a: 1,b: 2,2: 3}
// 数组
...
// in
  ...
  type PropKeys = 'x' | 'y' | 'z' // 基于索引签名类型
  type Type2 = { [Key in PropKeys]: number } // 表示key可以是PropKeys联合类型中任意一个 类似于forin(let k in obj)
// in keyof
  type Props = {a: number;b: string;c:boolean}
  type Type3 = {[Key in keyof Props]: number} // key为abc 类型都为number
// 泛型工具类都是基于映射类型实现的
  type Partial2<T> = {
    [P in keyof T]?: T[P] // keyof T即keyof Props2表示获取Props2所有的键 abc
    // ? 表示将这些属性变为可选 以此来实现Partial的功能
    // T[P] 表示获取T每个键对应的类型（abc）
